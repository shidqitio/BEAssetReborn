...
import laporanPersediaanService from "../../services/web/laporanpersediaan-service";
...
const getAll = async (
	...
	try {
		...
		const [assetpersediaan, err]: [BarangDetailResponse, IErrorResponse] = await laporanPersediaanService.getAll();
...
const getById = async (
	...
	next: NextFunction): Promise<void> => {
	try {
		const kodeUnit = req.params.kode_unit

		const [assetpersediaan, err]: [BarangDetailResponse, IErrorResponse] = await laporanPersediaanService.getById(kodeUnit);
